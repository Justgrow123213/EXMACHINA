...
def check_module(vals: List[str]) -> bool:
    ...
    logger = get_logger()
    logger.warning(vals)
    valid_modules = {m.val for m in Modules}
    return all(val in valid_modules for val in vals)
...
def load_workflow(workflow_path):
    try:
        ...
        logger = get_logger()
        logger.e(f"The file {workflow_path} was not found.")
        return None
    except json.JSONDecodeError:
        logger = get_logger()
        logger.e(f"The file {workflow_path} contains invalid JSON.")
        return None
